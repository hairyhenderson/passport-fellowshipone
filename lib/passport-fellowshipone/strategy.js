...
function Strategy(options, verify) {
   ...
   options.apiURL = options.apiURL || 'https://{churchCode}.fellowshiponeapi.com/v1'
   // Handle templating on apiURL
   options.apiURL = URI.expand(options.apiURL, options).normalize().toString()
   options.requestTokenURL = options.requestTokenURL || URI(options.apiURL).segment('Tokens/RequestToken').toString()
   options.accessTokenURL = options.accessTokenURL || URI(options.apiURL).segment('Tokens/AccessToken').toString()
   options.userAuthorizationURL = options.userAuthorizationURL || URI(options.apiURL).segment('PortalUser/Login').toString()
